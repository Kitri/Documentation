...
* Describe Core Azure services (15 - 20%)
 ...
 - Resources :: Instances of services that you create
 - Resource groups :: resources combined into groups that act as logical containers
 - Subscriptions :: groups together user accounts and resources
...
An account can have one or many subscriptions, that have different billing models and to which you apply different access-management policies.

2 Types of subscription boundaries that you can use:
 - Billing boundary :: Determined how an azure account is being billed
 - Access control boundary ::Azure applies access-management policies at the subscription level.

Might want to create azure subscriptions to separate: Environments, org structures or billing.

Subscriptions are bound to some hard limitations e.g. max number of ExpressRoute circuits per subscription is 10.
...
Many different subscriptions, and group them. Level of scope above subscriptions.

All subscriptions in management group inherits conditions applied to group.

Group tree (Azure supports max depth 6 excl root)
# #+ATTR_HTML: :width 800 
...
When you create a virtual machine you have to create a virtual network
 - Virtual Network :: Emulating a physical network. MS Global Network already exists, so a VN is just software config
   - Can link VNs together using network peering (allows resources in VNs to communicate with each other)
 - Virtual Private Network (VPN) Gateway :: Connecting two networks as if they were on the same network
   - Policy-based VPN - static routing (specify IPs)
 ...
 - ExpressRoute :: high-speed private connection to Azure (physical fibre connection to an actual Azure box)
   - Connectivity from 
     - any-to-any (integrate WAN with Azure) 
     - point-to-point ethernet 
     - virtual cross-connection via provider at colocation facility (e.g. CloudExchange colocation)
   - Does not go over the public internet
   - Not encrypted, just private
   - L3 connectivity between on-prem and cloud
   
Connectivity between resources:
 - Point-to-site :: Client computor connects via encrypted VPN connection to Azure VN
 - Site-to-site :: Links on-prem VPN to AZure VPN in a virtual network. Looks like local network and is encrypted and works over the internet
 - Network-to-network :: Connect one VN to another VN
   
Route network traffic:
 - Route tables :: define rules about how traffic should be directed
 - Border Gateway Protocol :: works with Azure VPN gateway or ExpressRoute to propagate on-prem BGP routes to Azure VN's

OSI Model:
 - Layer 2 (L2) :: Data Link Layer, provides node-to-node communication in the same network
 - Layer 3 (L3) :: Network Layer, provides adressing and routin gbetween nodes on multi-node network

Benefits of ExpressRoute
 - Built-in redundancy
 - Connectivity to MS Cloud Services
 - ExpressRoute Global Reach - exchange data across on-prem sites by connecting ER circuits
 - Dynamic routing
...
**** Storage Services
4 types of storage services: Blob, File, Queue and Table. 
 - Blob. Ideal for: 
   - storing images, documents; 
   - storing files for distributed access; 
   - streaming video and audio, storing data for backup/restore, DR and archiving; 
   - storing data for analysis by an on-prem; 
   - storing up to 8TB of data for VM
 - Files
   - Azure Files offers fully managed file shares in the cloud that are accessible via standard file sharing protocols
...
**** Blob Access Tiers
 ...
 - Hot access tier :: Optimized for storing data that is accessed frequently (e.g. website images)
 - Cool access tier :: Optimized for data that is infrequently accessed and stored for at least 30 days (e.g. invoices)
...
*** Databases
 ...
 - SQL Managed Instance :: (PaaS) Most compatible with existing SQL Server, fully managed by azure. Always up-to-date
...
*** Big Data and Analytics
 ...
 - Azure Data Lake Analytics :: on-demand analytics job service. Write queries to transform data and extract valuable insights. Only pay when it's running
...
*** AI
 ...
 - Azure Cognitive Services :: Prebuilt machine learning models that enable apps to see, hear, speak, understand and begin to reason. Language/Speech/Vision/Decision

*** Serverless
 - Azure Functions :: Functions can execute code in almost any modern language. Stateless (durable functions provide state). Code first
 - Logic Apps :: Designed in a web-based designer and can execute logic triggered by azure services without writing any code. Stateful. Designer-first (declarative) - executes workflows
 - Event Grid (used to be App Grid)
   
Azure Functions is intended for serverless computation and Logic apps for serverless orchestration

*** DevOps
    
 - Azure DevOps :: Azure Repos, - Boards, - Pipelines, - Artifacts, - Test Plans 
...
Recommendations divided into five categories:
 - Reliability :: Improve continuity of business-critical applications
 - Security :: Detect threats and vulnerabilitys
...
Personalised view of health of Azure services, regions and resources you rely on.

Event Types:
 - Service issues
 - Planned maintenance
 - Health advisories (e.g. service retirements and breaking changes. Announced far in advance)
...
Dashboard for security. Unified infrastructure security management
system that monitors and protects your systems inside and outside of
Azure. Visibility of security posture - Azure and on-prem

Security poster = cybersecurity policies and controls, as well as how well you can predict, prevent, and respond to security threats
...
** Describe Azure Network Security
*** Defence in depth
Security in more than one place. Layers are in order - physcial comes first, data last
 - Data :: Controlls access to data e.g. Virtual Network Endpoint
 - Application :: Helps ensure apps are secure and free of vulnerabilities e.g. API Management
 - Compute :: Secures access to VMs e.g. Limit remote desktop access, windows update
 - Network :: Limits comms between resources e.g. NSG, use of subnets, deny by default
 - Perimeter :: Uses DDoS protection to filter attacks e.g. DDoS, firewalls
 - Identity & Access :: Controls access to infrastructure and change control e.g. Azure AD
 - Physcial :: Protect hardware e.g. Door locks, key cards 
...
Stateful firewall. Analyses complete context of a network connection

Features high availability and unrestricted cloud scalability

Inbound Destination Network Address Translation (DNAT) support

Azure Application Gateway provides a firewall called Web Application Firewall (WAF) which provides centralized, inbound protection for web apps
...
Distributed Denial of Service attack
 ...
 - L7 protection with Application Gateway Web application firewall (L7 = application)
 - Globally deployed
...
RBAC is applied to a scope such as a management group, single subscription, resource group or single resource.
...
Useful for: 
 ...
 - Operations management (group resources according to availability criticality, can use this to create SLAs)
 - Security (e.g. public / confidential)
 - Governance and regulatory compliance (e.g. ISO xxx)
 - Workload optimization and automation (e.g. use ADO to perform tasks on tag)
...
*** Purpose of documents
 ...
 - Microsoft Privacy statement :: explains what personal data MS collections, how they use it and for what purposes@ privacy.microsoft.com
 - Online Service Terms (OST) :: Terms and conditions for use. To use their website you have to abide by this. legal agreement between MS and customer
 - Data Protection Addendum (DPA) :: How they handle retention of data

*** Trust Centre

Portal that talks about MS's compliance and standards. Principles for
maintaining data integrity in the cloud and how MS implements and
supports security, privacy, compliance and transparancy.
